                    ...
                    <div class="col-lg-6 col-md-6 mb-4" id="poli-{{ $poli->id ?? '-' }}">
                        <div class="service-item bg-white shadow-lg rounded h-100 p-4 position-relative"
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                ...
                                @if ($poli->nama != 'total')
                                    <span
                                        class="badge {{ $poli->current_antrian > 0 ? 'bg-success' : 'bg-danger' }} p-2 status-badge">
                                        {{ $poli->current_antrian > 0 ? 'Aktif' : 'Tidak Aktif' }}
                                    </span>
                                @endif
                            </div>
                            @if ($poli->nama != 'total')
                                <p class="mb-2 fs-5 text-center text-secondary">Sedang Dalam Pemeriksaan</p>
                                ...
                                <div class="d-inline-flex align-items-center justify-content-center bg-light rounded-circle shadow-sm mb-4"
                                    ...
                                    <h2 class="mb-0 fw-bold text-primary current-antrian">
                                        {{ sprintf('%03s', $poli->current_antrian) }}</h2>
                                </div>
                            @endif
                            <div class="text-center">
                                <p class="mb-2 fs-5 text-secondary">Total Antrian</p>
                                <p class="fw-bold text-dark fs-3 total-antrian">{{ sprintf('%03s', $poli->total_antrian) }}
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            $.ajax({
                ...
                success: function(response) {
                    response.forEach(function(poli) {
                        ...
                        if (poli.nama !== 'total') {
                            poliElement.find(".current-antrian").text(poli.current_antrian > 0 ?
                                sprintf(
                                    '%03', poli.current_antrian) : '---');
                        }

                        if (poli.nama !== 'total') {
                            // Update status badge
                            if (poli.current_antrian > 0) {
                                poliElement.find(".status-badge").removeClass('bg-danger').addClass(
                                    'bg-success').text('Aktif');
                                poliElement.find(".status-text").text('Aktif');
                                poliElement.find(".service-item").css('border-top',
                                    '4px solid #28a745');
                            } else {
                                poliElement.find(".status-badge").removeClass('bg-success').addClass(
                                    'bg-danger').text('Tidak Aktif');
